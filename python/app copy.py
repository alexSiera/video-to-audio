...
def optimize_audio(audio_path):
    ...
    try:
        ...
        with tqdm(total=100, desc="Optimizing audio", ncols=100) as pbar:
            ...
            # Noise reduction
            audio = audio.low_pass_filter(4000).high_pass_filter(100)
            pbar.update(20)
    ...
    try:
        ...
        with tqdm(total=total_chunks, desc="Preparing chunks", ncols=100) as pbar:
            ...
            for i in range(0, len(audio), chunk_length_ms - overlap):
                ...
                chunk = chunk.fade_in(100).fade_out(100)
                chunks.append(chunk)
                pbar.update(1)
        return chunks
    except Exception as e:
...
def transcribe():
    ...
    try:
        ...
        ordered_transcription = " ".join([text for text in results if text.strip()])
